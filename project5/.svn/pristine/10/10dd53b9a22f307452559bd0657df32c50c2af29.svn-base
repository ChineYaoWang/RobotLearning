...
def train(args):
    ...
    checkpoint_callback = CheckpointCallback(save_freq=16384, save_path='logs/',name_prefix='rl_model')
    ...
    #for i in range(10):
    model.learn(total_timesteps=1343488 ,callback=checkpoint_callback)
    
    model.save(f"{args.save_dir}/model")
